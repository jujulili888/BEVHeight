...
def result2kitti(results_file, results_path, dair_root, gt_label_path, demo=False):
    ...
    for sample_token in tqdm(results.keys()):
        print("xxxxxxxxxxxxxxxxxxxxx",sample_token)
...
def result2kitti_rope3d(results_file, results_path, dair_root, gt_label_path, demo=False):
    ...
    with open(results_file,'r',encoding='utf8')as fp:
        results = json.load(fp)["results"]
    print("xxxxxxxxxxxxxxxxxxxx",dair_root, gt_label_path)
    id_pth = gt_label_path.split("/")[0]+'/'+ gt_label_path.split("/")[1]
    print(id_pth)
    id_file = os.path.join(id_pth, "map_token2id.json")
    with open(id_file) as fp:
            token2sample = json.load(fp)
    # if 'dair-in-rope3d' in dair_root:
    #     with open("data/dair-in-rope3d-kitti/map_token2id.json") as fp:
    #         token2sample = json.load(fp)
    # else:
