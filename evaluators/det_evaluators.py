...
class RoadSideEvaluator():
    ...
    def evaluate(
        ...
        result_files, tmp_dir = self.format_results(results, img_metas,
                                                    result_names,
                                                    jsonfile_prefix)
        print('#####################result_fils#####################',result_files, tmp_dir)
        results_path = "outputs/metrics_semi_dair" 
        if 'dair' in self.data_root and 'rope' not in self.data_root and "gen" not in self.data_root:
        ...
        kitti_evaluation(pred_label_path, self.gt_label_path, current_classes=self.current_classes, metric_path="outputs/metrics_semi_dair")
