    ...
    def get_image(self, cam_infos, cams):
        ...
        for cam in cams:
            ...
            for sweep_idx, cam_info in enumerate(cam_infos):
                ...
                intrin_mat_ori, sweepego2sweepsensor_ori = intrin_mat.clone(), sweepego2sweepsensor.clone()
                ...
                if self.return_depth and sweep_idx == 0:
                    ...
                    object_embed = np.load(os.path.join(self.data_root, 'object_embeddings', 'mean', file_name.replace("jpg", "npy")), allow_pickle=True)
                    object_depth = cv2.imread(os.path.join(self.data_root, 'mask_image', file_name.replace("jpg", "jpg")))                    

                    object_depth = object_depth[:,:,0][:, :, np.newaxis]
                    object_embed = np.transpose(object_embed[0], (1,2,0))
                    object_embed, object_depth = \
                        self.mask_embed_intrin_extrin_transform(object_embed, object_depth, intrin_mat_ori.numpy(), sweepego2sweepsensor_ori.numpy(), intrin_mat.numpy(), sweepego2sweepsensor.numpy())
                    object_depth = torch.tensor(object_depth)
                    object_embed = torch.tensor(object_embed)
                    ...
                    '''
                    gt_depth.append(object_depth)
                    gt_embed.append(object_embed)
        ...
        if self.return_depth:
            ...
            ret_list.append(torch.stack(gt_embed))
        return ret_list
