...
class LSSFPN(nn.Module):
    def __init__(self, x_bound, y_bound, z_bound, d_bound, final_dim,
        ...
        self.assist_layer = nn.Conv2d(512, 256, kernel_size=1, stride=1, padding=0)
        '''
        self.segment_layer = nn.Sequential(
            nn.ConvTranspose2d(512, 512 // 4, kernel_size=2, stride=2),
            nn.BatchNorm2d(512 // 4),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(512 // 4, 512, kernel_size=2, stride=2),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
    ...
    def _forward_single_sweep(self,
        ...
        # segment_features = self.segment_layer(source_features)
        assist_features = self.assist_layer(source_features)

        height_feature = self._forward_height_net(source_features, mats_dict)
        ...
        if is_return_height:
            return feature_map.contiguous(), (assist_features, assist_features)
