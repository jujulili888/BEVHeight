...
class BEVHeightLightningModel(LightningModule):
    ...
    def train_dataloader(self):
        train_dataset = NuscMVDetDataset(
            ...
            info_path=os.path.join(data_root, 'dair_12hz_infos_train_new_split.pkl'),
            is_train=True,
    ...
    def val_dataloader(self):
        val_dataset = NuscMVDetDataset(
            ...
            classes=self.class_names,
            data_root=self.data_root,
            info_path=os.path.join(data_root, 'dair_12hz_infos_val_new_split.pkl'),
            is_train=False,
            img_conf=self.img_conf,
            num_sweeps=self.num_sweeps,
            sweep_idxes=self.sweep_idxes,
...
def main(args: Namespace) -> None:
    ...
    checkpoint_callback = ModelCheckpoint(dirpath='./outputs/bev_height_lss_r50_864_1536_128x128-newsplit/checkpoints', filename='{epoch}', every_n_epochs=5, save_last=True, save_top_k=-1)
    ...
    if args.evaluate:
        ...
        backup_codebase(os.path.join('./outputs/bev_height_lss_r50_864_1536_128x128-newsplit', 'backup'))
        trainer.fit(model)
...
def run_cli():
    ...
    parser.set_defaults(
        ...
        precision=32,
        default_root_dir='./outputs/bev_height_lss_r50_864_1536_128x128-newsplit')
