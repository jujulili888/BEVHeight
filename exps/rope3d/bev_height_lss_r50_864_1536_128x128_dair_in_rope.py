...
data_root = "data/dair-in-rope3d/"
gt_label_path = "data/dair-in-rope3d-kitti/training/label_2"
...
class BEVHeightLightningModel(LightningModule):
    ...
    def get_depth_loss(self, gt_depths, gt_embeds, depth_preds):
        segment_features, assist_features = depth_preds
        gt_depths = gt_depths.permute(0, 1, 4, 2, 3)[:, 0]
        gt_embeds = gt_embeds.permute(0, 1, 4, 2, 3)[:, 0]
        embed_loss = F.mse_loss(gt_embeds, assist_features)
        '''
        segment_features = F.interpolate(segment_features, (gt_depths.shape[-2], gt_depths.shape[-1]), mode="bilinear", align_corners=False)
        depth_loss = self.dice_loss(gt_depths, segment_features)
        '''
        return embed_loss * 1000

    # def training_step(self, batch):
    #     (sweep_imgs, mats, _, _, gt_boxes, gt_labels, gt_depths, gt_embeds) = batch
    ...
    #     depth_loss = self.get_depth_loss(gt_depths, gt_embeds, depth_preds)
    #     self.log('detection_loss', detection_loss)
    #     self.log('depth_loss', depth_loss)
    #     if gt_labels[0].shape[0] == 0 or gt_labels[1].shape[0] == 0:
...
def main(args: Namespace) -> None:
    ...
    checkpoint_callback = ModelCheckpoint(dirpath='./outputs/bev_height_lss_r50_864_1536_128x128_dair_in_rope/checkpoints', filename='{epoch}', every_n_epochs=5, save_last=True, save_top_k=-1)
    ...
    if args.evaluate:
        ...
        if args.ckpt_path is None:
            backup_codebase(os.path.join('./outputs/bev_height_lss_r50_864_1536_128x128_dair_in_rope', 'backup'))
            trainer.fit(model)
        else:
...
def run_cli():
    ...
    parser.set_defaults(
        ...
        precision=32,
        default_root_dir='./outputs/bev_height_lss_r50_864_1536_128x128_dair_in_rope')
