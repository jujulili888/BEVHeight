...
def parse_option():
    ...
    parser.add_argument('--dest-root', type=str, default="data/gen_dair2", help='root path to result rope3d dataset')
...
if __name__ == "__main__":
    robutness = Robutness()
    args = parse_option()
    split_root = 'data/dair-in-rope3d-newsplit'
    ...
    raw_frame_ids_ori = read_split(os.path.join(split_root, "raw.txt"))
    print(len(raw_frame_ids_ori),raw_frame_ids_ori[0], type(raw_frame_ids_ori))
    
    raw_frame_ids = []
    calib_path = os.path.join(src_root, "training", "calib")
    for idx in range(len(raw_frame_ids_ori)):
        index = raw_frame_ids_ori[idx]
        calib_file = os.path.join(calib_path, index + ".txt")
        camera_intrinsic = load_calib(calib_file)
        if camera_intrinsic == 2757.839717:
            raw_frame_ids.append(index)
    print(len(raw_frame_ids),raw_frame_ids[0], type(raw_frame_ids))
    ...
    raw_frame_ids = random.sample(raw_frame_ids, 4000)
    ...
    for raw_id in tqdm(raw_ids):
        ...
        for train_id in tqdm(train_frame_ids_gen):
            ...
            '''
            sample_info_combined = frame_combine_tools(robutness, src_root, train_id, raw_id, count, vis)
