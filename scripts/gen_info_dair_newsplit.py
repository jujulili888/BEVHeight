...
def new_split(dair_root):
    info = mmcv.load("data/single-infrastructure-split-data.json")
    split_list = info['train'] + info['val']
    split_by_cam = list()
    for sample_id in tqdm(split_list):
        camera_intrinsic_path = os.path.join(dair_root, "calib", "camera_intrinsic", sample_id + ".json")
        camID = get_camID(camera_intrinsic_path)
        split_by_cam.append(camID)
    split_by_cam = list(set(split_by_cam))
    new_split_dict = {i:[] for i in split_by_cam}
    for sample_id in tqdm(split_list):
        camera_intrinsic_path = os.path.join(dair_root, "calib", "camera_intrinsic", sample_id + ".json")
        camID = get_camID(camera_intrinsic_path)
        new_split_dict[camID].append(sample_id)
    
    mmcv.dump(new_split_dict, "data/dair-v2x-i/new_split.json")
    
    return new_split_dict


def generate_info_dair(dair_root, split):    
    infos = mmcv.load("data/dair-v2x-i/new_split.json")
    print('camera1', len(infos['camera1']), 'camera3', len(infos['camera3']), 'camera5', len(infos['camera5']))
    if split == 'train':
        split_list = infos['camera1'] + infos['camera5']
    elif split == 'val':
        split_list = infos['camera3']
    infos = list()
    for sample_id in tqdm(split_list):
...
def main():
    ...
    # mmcv.dump(train_infos, './data/dair-v2x-i/dair_12hz_infos_train_new_split.pkl')
    # mmcv.dump(val_infos, './data/dair-v2x-i/dair_12hz_infos_val_new_split.pkl')
